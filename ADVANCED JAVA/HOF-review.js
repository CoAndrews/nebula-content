...
const largerThan100 = nums.find(el => el > 100);

console.log(largerThan100)
...
const people = [
    ...
    console.log(people)
// 3.   Given an array of cars only list cars that are built pre-2000
const cars = [{model: "Dodge", make: 2001},{model:"Ford", make:2012},{model:"Honda", make:1989},{model:"Hondi",make:1990}]

const pre2000 = cars.filter(cur => cur.make < 2000);
...
const protected = phoneNums.map(hidden)
...
nums1.forEach(el => el  > 1000 ? console.log(`Thats a big number!`): console.log(`This number isnt so big...`))

// 6.   Given an array of people's names write a hof that returns only people with names longer than 5 characters
//people1.sort((a, b) => (a.name.length > b.name.length) ? 5 : -1);
const peopleNames = ['Harry', 'Langston', 'Deyvn']
    const longNames = peopleNames.filter(cur => cur.length >5);
